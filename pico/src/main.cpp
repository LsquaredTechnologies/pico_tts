...
int main(int argc, char *argv[])
{
	...
	char* outputLang = NULL;

	fprintf(stderr, "pico2wave\n");
	...
	while ( (currentOption = getopt(argc, argv, "hl:o:")) != -1)
	{
		...
		case 'l':
			outputLang = optarg;
			fprintf(stderr, "Output language set to '%s'\n", outputLang);
			break;
		case 'o':
			outputFilename = optarg;
			fprintf(stderr, "Output audio set to '%s'\n", outputFilename);
			break;
		default:
	...
	result = ttsEngine->setLanguage("eng", "ENG", "");
